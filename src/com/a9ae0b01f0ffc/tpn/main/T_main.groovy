...
class T_main {
    ...
    private static void spawn_thread(GPathResult i_thread_xml) {
        T_tpn_thread l_tpn_thread = new T_tpn_thread(i_thread_xml.@name.text(), i_thread_xml.@endpoint.text(), Integer.parseInt(i_thread_xml.@thread_number.text()), i_thread_xml.@mode.text(), i_thread_xml.@thread_config_file_name.text())
    ...
    static void main(String... i_args) {
        ...
        T_tpn_s.x().sql_update("update messages set thread_number=null where thread_number is not null")
        ...
        while (T_tpn_const.GC_TRUE) {
            ...
            for (String l_source_to_update in p_threads_by_source_by_number.keySet()) {
                T_tpn_s.x().sql_update("update messages set thread_number=FLOOR( 1 + RAND( ) * ${p_threads_by_source_by_number.get(l_source_to_update).keySet().size()} ) where thread_number is null")
                T_tpn_s.x().get_sql().commit()
            }
